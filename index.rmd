...
# Rainfall and temperature analysis and prediction for Perth Airport
...
Fortunately, we can see that taking mean values, such as mean yearly maximum temperature in a given month results in a more normally distributed variable. Although it is still somewhat irregular and there appears to be a few outliers.
...
temperature.yearly.avg %>% 
  ...
  geom_smooth(method = "lm") +
  dark_theme_bw() +
  ggtitle("Yearly mean min and max temperatures for Perth Airport (1944-2022)")
temperature.yearly.avg %>% 
...
```{r}
models.temperature.month.by.year.avg %>% 
  filter(term == "Year") %>% 
...
monthly.temp.trend.graph <- temperature.month.by.year.avg %>% 
  mutate(temp_range = meanTMAX-meanTMIN) %>% 
...
Yearly min and max regardless of month?
...
A quick look at aggregated Monthly 

```{r}
temperature.month.by.year.avg %>% 
  mutate(temp_range = TMAX-TMIN) %>% 
  group_by(Year) %>% summarise(mean_temp_range = mean(temp_range, na.rm = T)) %>% 
  ggplot(aes(Year, mean_temp_range)) + geom_point() + dark_theme_bw() +geom_smooth() +
  ggtitle("Mean yearly temperature range for Perth Airport (1944-2022)")
```
...
The estimates can be interpreted as follows. The intercept gives us our baseline rainfall total in 1944 (over 2000mm)

```{r}
model.rainfall.vs.year.meanTMAX.interaction <- lm(rain_total ~ base_year*meanTMAX, data = temperature.yearly.avg)
model.rainfall.vs.year.meanTMAX.interaction %>%
    summary %>% broom::tidy() %>% 
    mutate(across(c("estimate", "std.error", "statistic", "p.value"), round, 2))
model.rainfall.vs.year.meanTMAX <- lm(rain_total ~ base_year+meanTMAX, data = temperature.yearly.avg)
model.rainfall.vs.year.meanTMAX %>%
    summary %>% broom::tidy() %>% 
    mutate(across(c("estimate", "std.error", "statistic", "p.value"), round, 2))
```
...
```{r}
temperature.yearly.avg %>% 
  ggplot(aes(meanTMAX, rain_total)) +
  geom_point() +
  geom_smooth(method="lm") +
  dark_theme_bw()
temperature.yearly.avg %>% 
  ggplot(aes(base_year, rain_total)) +
  geom_point() +
  geom_smooth(method="lm") +
  dark_theme_bw()
...
```{r}
lm(rain_total ~ Year*meanTMAX*Month, data = temperature.month.by.year.avg) %>% 
  summary %>% broom::tidy()
```
...
temperature.month.by.year.avg %>% 
    filter(Month == 7) %>% 
    lm(rain_total ~ meanTMAX, data =.) %>% 
    summary
```
```{r}
temperature.month.by.year.avg %>% 
    filter(Month == 4) %>% 
    ggplot(aes(meanTMAX, rain_total, colour=Year)) +
    geom_point() +
    geom_smooth(method="lm") +
    dark_theme_bw()
```
